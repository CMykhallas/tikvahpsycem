...
async function testPriceValidation() {
  ...
  try {
    ...
    assert(isRejected(result) || Boolean(result.data?.tamperedProducts || result.data?.tampered));
  ...
  try {
    ...
    assert(result.ok);
    logTest("Tolerância de arredondamento", true, "Pequenas variações aceitas", result.data);
  } catch (error) {
  ...
  try {
    ...
    assert(isRejected(result) || Boolean(result.data?.tampered));
...
async function testRateLimiting() {
  ...
  for (let i = 0; i < 10; i++) {
    try {
      ...
      logTest(`Rate limit tentativa ${i + 1}`, false, `Erro: ${errorMessage(error)}`);
    }
  }
  ...
  logTest(
    ...
    { successCount, windowMs: 15 * 60 * 1000, maxRequests: 5 }
  );
}
...
async function testDatabaseSecurity() {
  ...
  try {
    ...
    assert(isRejected(result) || !String(result.data?.customer_email ?? "").includes("DROP"));
  ...
  try {
    ...
    assert(isRejected(result) || !String(result.data?.customer_email ?? "").includes("<script>"));
  ...
  try {
    ...
    assert(isRejected(result) || !result.data?.role);
...
async function testTokensAndSessions() {
  ...
  try {
    const result = await fetchAPI("/functions/v1/create-checkout", "POST", validCheckout);
    const hasToken = Boolean(result.data?.token || result.data?.order_token);
    assert(result.ok);
    assert(hasToken);
    logTest("Token de order gerado", true, "Cliente recebe token único", { token: "Presente" });
  } catch (error) {
